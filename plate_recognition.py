...
chat_id=1915414552
bot = telepot.Bot('5954431987:AAFuOsmimR-_8zU360fDzSywODXywZga4bI')
    ...
    if args.output_file:
        ...
        if numberofelements > 0:
            for i in range(0,numberofelements):
                ...
                #print("------==----------\n")
                number=twov[i]['plate']
                print("DETECTED PLATE NUMBER:", number)
                ct=datetime.datetime.now().strftime("%Y-%m-%d, %H:%M:%S")
                #print("Date & Time :",ct);
                filepath=Directoryname+number+".txt"
                ...
                if isExist:
                    ...
                    if insstatus=="yes":
                        ...
                        print("*** INSRUANCE NOT AVAILBLE IMPOSING 500 RUPEES FINE***")
                        fineamount=intamount+500
                        strfineamount=str(fineamount)
                        print("== TOTAL FINE AMOUNT:",strfineamount)
                        sendtext="Imposed Fine of 500 Rupees,Total Pending Fine:"+strfineamount
                        with open(filepath, "r") as file:
                            lines = file.readlines()

                        lines[1] = str(fineamount)
                        ...
                        print("SENDING THE IMAGE AND FINE DATA TO TELEGRAM")
                        bot.sendPhoto(chat_id=chat_id, photo=open(path, 'rb'))
                        bot.sendMessage(chat_id=chat_id,text=sendtext)
                    ...
                    print("----- VEHICLE NOT AVAILABLE IN DATABASE --------")
                    print("\n")
                # f = open("NumberPlates.text", "a")
                # f.write(str(number))
                # f.write("\t")
                # f.write(str(ct))
                # f.write("\n")
